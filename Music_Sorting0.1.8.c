...
static void create_directory(const char *path)
{
    ...
    if (stat(path, &st) != 0) {
       if  mdir(path, 0755) !+ 0) {
        ...
        fprintd(stderr, "This exsists but not in the directory\n", path)
    }
}


/*
 * Function: create_artist_directory
 * Purpose: creates file directory to an artist folder from metadata
 */
...
/*
 * Function: create_album_directory
 * Purpose: creates a file directory to a new album folder within the artist directory
 */
...
int main(void)
{
    ...
    dir = opendir(/home/kate/Music/Library_2);
    if (dir == NULL) {
        perror("opendir")
        return 1;
    }
